...
Vec3f cast_ray(const Vec3f &orig, const Vec3f &dir, const Objects &objs, const Lights &lights, const Environment& env, unsigned int depth = 0) {
  ...
  if(!scene_intersect(orig, dir, objs, hit_point, hit_material, hit_normal)) return env.map(orig, dir);
    ...
    Vec3f refraction_vec = dir + (-hit_normal)*hit_material.refraction_index; 
...
void render(const Viewport& view, const Objects &objs, const Camera &cam, const Lights &lghts, const Environment& env, const string& filename){
  ...
  for(int i = 0; i < view.ny; i++){
    for(int j = 0; j < view.nx; j++){
      ...
      dir.normalize();
      dir = dir*cos(cam.roll) + cross(cam.dir, dir)*sin(cam.roll) + cam.dir*(cam.dir*dir)*(1-cos(cam.roll)); 
      dir.normalize();
...
int main() {
  ...
  Model m("./piramida.obj", 1, Vec3f(2, 5, -9), red);
  
  
  Objects objs = { &surface, &c1, &s1,  &m};
  ...
  Viewport view2(1024, 768, 2.14/2);

  Camera cam(Vec3f(0,0,0), Vec3f(0,0,-1), 0);
  Camera cam2(Vec3f(9,3,0), Vec3f(-1,0,-1), 0);

  

  Environment env("./env.ppm", 1500, 2880, 1800);

  render(view, objs, cam, lights, env, "./s.ppm");
  render(view, objs, cam2, lights, env, "./l.ppm");
  render(view2, objs, cam, lights, env, "./k.ppm");
